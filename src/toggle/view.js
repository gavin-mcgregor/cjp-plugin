	...
	allToggles.forEach((toggle) => {
		toggle.addEventListener("click", (e) => {
			...
			if (!targetToggle.classList.contains("open")) {
				...
				const newHeight =
					parseInt(targetToggle.getAttribute("data-open")) + 6 * 8;
				targetToggle.style.height = `${newHeight}px`;
				content.classList.add("visible");
			} else {
				targetToggle.classList.remove("open");
				const newHeight =
					parseInt(targetToggle.getAttribute("data-close")) + 4 * 8;
				targetToggle.style.height = `${newHeight}px`;
...
function calculateToggleHeights() {
	...
	toggleSingles.forEach((toggleSingle) => {
		...
		toggleSingle.style.height = `${
			parseInt(toggleSingle.getAttribute("data-close")) + 4 * 8
		}px`;
	});
}
